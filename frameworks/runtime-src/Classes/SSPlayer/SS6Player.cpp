...
std::vector<std::string> ResourceManager::getAnimeName(const std::string& dataKey, const std::string& packName)
{
    ...
    while (1)
    {
        ...
        if (itpairstri->second->packName == packName) { // 指定されたssae名と異なる場合はスキップ
            animename.push_back(itpairstri->second->animeName);
        }
        itpairstri++;
...
// 指定したパーツの子パーツも含めて表示・非表示を切り替える。
void Player::setPartVisibleRecursive(std::string partsname, bool flg) {
    if (_currentAnimeRef) {
        ToPointer ptr(_currentRs->data);
        ...
        for (int index = 0; index < packData->numParts; index++) {
            ...
            if (strcmp(partName, partsname.c_str()) == 0) {
                _partVisible[index] = flg;
                parentIndexes.push_back(partIndex);
                break;
            }
        }
        // 全パーツをみて、該当親パーツを持っていたらflgも親パーツに設定したものを設定する。
        while (true) {
            std::vector<int> nextParentIndexes;
            for (int index = 0; index < packData->numParts; index++) {
                int partIndex = _partIndex[index];
                const PartData* partData = &parts[partIndex];
                if (std::find(parentIndexes.begin(), parentIndexes.end(), partData->parentIndex) != parentIndexes.end()) {
                    _partVisible[index] = flg;
                    nextParentIndexes.push_back(partIndex);
                }
            }
            if (!nextParentIndexes.empty()) {
...
std::string Player::getPartCellName(std::string partsname) {
    if (_currentAnimeRef) {
        ToPointer ptr(_currentRs->data);
        const Cell* cells = static_cast<const Cell*>(ptr(_currentRs->data->cells));
        ResluteState rs;
        getPartState(rs, partsname.c_str(), getFrameNo());
        const Cell* cell = &cells[rs.cellIndex];
        const char* name1 = static_cast<const char*>(ptr(cell->name));
        return std::string(name1);
    }
    return "";
}

// インスタンスパーツが再生するアニメを変更します。
bool Player::changeInstanceAnime(std::string partsname, std::string dataname, std::string animename, bool overWrite, Instance keyParam)
{
    //名前からパーツを取得
    bool rc = false;
    if (_currentAnimeRef)
        ...
        for (int index = 0; index < packData->numParts; index++)
        {
            ...
            if (strcmp(partName, partsname.c_str()) == 0)
            {
                ...
                if (sprite->_ssplayer)
                {
                    ...
                    {
                        if (!dataname.empty()) {
                            sprite->_ssplayer->setData(dataname);
                        }
...
{
    return changeInstanceAnime(partsname, "", animename, overWrite, keyParam);
}
