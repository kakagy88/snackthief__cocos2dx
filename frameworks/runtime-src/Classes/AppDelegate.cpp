...
#include "JsBindings/Magic.h"
#include "JsBindings/NetworkImageLoader.h"
#include "JsBindings/SsAvatarNode.hpp"
#include "JsBindings/SsPlayer.hpp"
#include "JsBindings/SsQuestBattleNode.hpp"
#include "JsBindings/SsRaidBattleNode.hpp"
#include "JsBindings/SsRaidTopNode.hpp"
#include "JsBindings/Downloader.hpp"
...
bool AppDelegate::applicationDidFinishLaunching()
{
    ...
    if(!glview) {
        ...
        glview = cocos2d::GLViewImpl::create("magic");
#else
        glview = cocos2d::GLViewImpl::createWithRect("magic", Rect(0,0,1136,640));
#endif
        director->setOpenGLView(glview);
    }
    NetworkImageLoader::getInstance()->initialize();
...
#endif
    sc->addRegisterCallback(register_all_magic);
    sc->addRegisterCallback(register_all_network_image_loader);
    sc->addRegisterCallback(register_all_ss_avatar_node);
    sc->addRegisterCallback(register_all_ss_player);
    sc->addRegisterCallback(register_all_ss_quest_battle_node);
    sc->addRegisterCallback(register_all_ss_raid_battle_node);
    sc->addRegisterCallback(register_all_ss_raid_top_node);
    sc->addRegisterCallback(register_all_downloader);
    sc->start();
...
void AppDelegate::createCacheDirectory() {
    auto fu = FileUtils::getInstance();
    // アバター画像保存用ディレクトリ作成
    auto avatarImageDirectoryPath = fu->getWritablePath() + "magic/avatarImage/";
    if (!fu->isDirectoryExist(avatarImageDirectoryPath)) {
        fu->createDirectory(avatarImageDirectoryPath);
    }
    // SpriteStudio用データ保存用ディレクトリ作成
    auto equipmentDirectoryPath = fu->getWritablePath() + "magic/equipment/";
    if (!fu->isDirectoryExist(equipmentDirectoryPath)) {
        fu->createDirectory(equipmentDirectoryPath);
    }
    NetworkImageLoader::getInstance()->initialize();
}

void AppDelegate::removeCacheDirectory() {
    auto fu = FileUtils::getInstance();
    fu->removeDirectory(fu->getWritablePath() + "magic");
    NetworkImageLoader::getInstance()->clearCache();
}
